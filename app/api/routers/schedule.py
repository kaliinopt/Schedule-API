...
async def get_schedule_for_date(
    ...
    schedule = await db.execute(select(model).where(model.date == date))

    return schedule.scalars().all()


@router.get(
    "/{audience_id}/week/{start_date}", response_model=List[schemas.ScheduleResponse]
)
    ...
    query = select(model).where(
        or_(
            ...
            and_(
                ...
                model.date <= end_date,
                or_(model.repeat_until.is_(None), model.repeat_until >= start_date),
            ),
        )
    )
...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.ScheduleResponse
)
...
async def update_schedule(
    ...
    if any(
        field in update_schedule.model_dump(exclude_unset=True)
...
async def delete_schedule(
    ...
    await db.commit()
    loger.info(
        f"Была удалена запись с id: {event.id} из таблицы {model.__tablename__} с аккаунта {admin.username}"
    )
