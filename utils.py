...
async def check_recurring_conflicts(
    ...
    check_to = new_event.repeat_until if new_event.repeat_until else new_event.date + timedelta(days=180)
    
    conditions = [
            model.repeat_frequency.is_not(None),
            or_(
                model.repeat_until.is_(None),
                model.repeat_until >= check_from
            )
    ]
    ...
    for event in existing_events:
        ...
        for event_date in event_dates:
            if (event_date == new_event.date or 
                (new_event.repeat_frequency and 
...
async def check_time_conflicts(
    ...
    if new_event.start_time >= new_event.end_time:
        raise HTTPException(status_code=400, detail="Конечное время должно быть позже начального")
